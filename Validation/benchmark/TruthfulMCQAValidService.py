...
class TruthfulMCQAValidService(IValidService):
    ...
    def preprocess_data(self, row) -> dict:
        common_system_prompt = "Choose the most likely answer."
        #####
        row_question = row['question']
        row_choices = row['choices']
        
        # Prepare the candidate prompt using the choices
        row_choices_prompt = "\n".join([f"{chr(ord('A') + i)}. {choice}" for i, choice in enumerate(row_choices)])
        row_user_prompt = "\n".join([row_question, row_choices_prompt])
        ...
        return {
            ...
            "label": [chr(ord('A') + i) for i in range(len(row_choices))],  # Generate label list (A, B, C, ...)
